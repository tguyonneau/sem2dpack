...
I = [0, 15, 30, 45, 60]
N = 5
dataset_directory = "../02_OUTPUT/Dataset20/"
...
def process_station_data_simplified(SEM):
    ...
    U_input = compute_displacement(V_input, time_input)
    ...
    for i in range(0,Time_stations.size):
        try : 
            if i==0:
                X_snap, Z_snap, Vx = load_field(SEM, 'x', i)
                _, _, Vz = load_field(SEM, 'z', i)
                Vx = np.expand_dims(Vx, axis=1)
                Vz = np.expand_dims(Vz, axis=1)
            else :
                _, _, Vx_temp = load_field(SEM, 'x', i)
                _, _, Vz_temp = load_field(SEM, 'z', i)
                Vx = np.concatenate([Vx, np.expand_dims(Vx_temp, axis=1)], axis=1)
                Vz = np.concatenate([Vz, np.expand_dims(Vz_temp, axis=1)], axis=1)
        except FileNotFoundError:
            break
    Ux, Uz, Ax, Az = [], [], [], []
    for s in range(0,Vx.shape[0]):
        Vxi, Vzi = Vx[s,:], Vz[s,:]
    ...
    # Compute PGV and PGA
    PGDx = np.max( np.max(np.abs(Ax), axis=1) ) 
    PGDz = np.max( np.max(np.abs(Az), axis=1) ) 
    PGVx = np.max( np.max(np.abs(Vx), axis=1) )
    PGVz = np.max( np.max(np.abs(Vz), axis=1) )
    PGAx = np.max( np.max(np.abs(Ax), axis=1) ) 
    PGAz = np.max( np.max(np.abs(Az), axis=1) ) 
    

    # Create a dictionary to hold the components of the stations data
    components = {}
    components['PGD_input'] = np.max(np.abs(U_input))
    components['PGDx'] = PGDx
    components['PGDz'] = PGDz
    components['PGV_input'] = np.max(np.abs(V_input))
    components['PGVx'] = PGVx
    components['PGVz'] = PGVz
    components['PGA_input'] = np.max(np.abs(A_input))
